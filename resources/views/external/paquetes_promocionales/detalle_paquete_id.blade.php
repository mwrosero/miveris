...
<script>
    ...
    dataCita.paquete = {
        "codigoEmpresaPaquete": 1,
        "codigoPaquete": {{ $idPaquete }}
    }
    ...
    async function obtenerPaquete(){
        let args = [];
        args["endpoint"] = api_url + `/${api_war}/v1/comercial/paquetes?canalOrigen=${canalOrigen}&codigoEmpresa=1&tipoFiltro=POR_ASIGNAR&page=1&perPage=500&estaPagado=false&verDetalle=false&categoria=&buscarPorPromocion=`;
        args["method"] = "GET";
        args["showLoader"] = true;
        const data = await call(args);

        $.each(data.data.items ,function(key, value){
            if(parseInt(value.codigoPaquete) == parseInt("{{ $idPaquete }}")){
                dataCita.paquete = value;
            }
        })
